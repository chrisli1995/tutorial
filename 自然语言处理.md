...
再往后就可以学习各种应用啦，情感分析，文本分类等，这个可以上斯坦福的那门NLP课程，也是非常棒的课程。（摘自知乎）

笔记中包含了NLP的学习以及所设计到语义关系的一些技术，比如社交网络等。**默认有一定的基础**

视频连接或百度云：

链接：https://pan.baidu.com/s/1sCUtrTNDwLVI7nwGvsMu3Q 密码：jxso（主讲RNN和seq2seq）
...
可以使用LTP（哈工大语言技术平台）对应的python工具包，安装方式在这里不做说明附上角色类型表。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-862dba947e56e164.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-62de04bb5de834d9.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

返回结果 roles 是关于多个谓词的语义角色分析的结果。由于一句话中可能不含有语义角色，所以结果可能为空。role.index 代表谓词的索引， role.arguments 代表关于该谓词的若干语义角色。arg.name 表示语义角色类型，arg.range.start 表示该语义角色起始词位置的索引arg.range.end 表示该语义角色结束词位置的索引。

一个简单的列子：输入“国务院 总理 李克强 调研 上海 外高桥 时 提出 ， 支持 上海 积极 探索 新 机制 。”，这是一个经过分词的句子，其语义角色标注结果为7 TMP:(0,6)A1:(9,14)，谓语的索引为7，7代表的“提出”，这个谓语有两个语义角色，第一个为类型为TMP，即时间，范围为（0,6）;第二个语义角色类型为A1,即动作的影响，范围为(9,14)。总结意思为： 在“**国务院总理李克强调研上海外高桥时**”这个时间“**提出**”了“**支持上海积极探索新机制**”。

### 0.2 依存句法分析

可以使用pyltp中间的Parser模块，parser.parse(words, postags)返回的结果有两个属性：head 表示依存弧的父节点词的索引。ROOT节点的索引是0，第一个词开始的索引依次为1、2、3…。

relation 表示依存弧的关系。具体的关系如下表所示：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-867aff1222b8b457.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![image.png](https://upload-images.jianshu.io/upload_images/7810235-7691dc2e688cf8fc.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

其次，对于领域隔离也是一个难点所在，不同领域的不同情况，机器的理解还是完全跟不上的，跨过任何一个领域都比较困难，一种新颖的观点是语言自己就是最好的表示。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-86ee256951e73f70.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

再然后就是数据集的搜索难度较大，比如少数民族的语言或者被和谐的语言。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-986500fc37212305.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

最后很多情况下无法评判结果的好坏，比如机器翻译，一句话有多种意思都是对的，不能说哪个翻译是对的哪个是错的。  ![image.png](https://upload-images.jianshu.io/upload_images/7810235-9e18ed70d897b458.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-ad9fdd54013bfe19.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上面的流程是概率系统如何解决语义理解的问题，其中主要很多技术现在可以通过深度学习的知识进行替换，主要的工作方式可以分解为：流程设计->收集训练数据->预处理->抽取特征->分类器->预测->评价这几步。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-20535b1e9839552c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

思考问题的时候可以以这个思路去做，比如在之前想做小说的人物关系图谱：

我的思路：阅读小说->找到小说中有哪些人物->通过文章中的内容判断哪些人物有关系->绘制图谱

机器的思路：阅读小说：按照分句以及分词的思路对小说的每一句话进行读取。

​	   找到小说中有哪些人物：序列标注问题，对小说每一个句子进行序列标注，找到人名。

​	   通过文章中的内容判断哪些人物有关系：关系抽取技术，对已标注关系的句子进行学习，				                通过深度学习的方式找到人物关系。

​	   绘制图谱：人为绘制还是显得有些麻烦，考虑使用知识图谱的技术，将关系结果放入数据库进行自动绘制。
...
如上图所示，式子反应一句话出现的概率，所有话出现的概率总和为1。比如P(隔壁老王)>P(隔壁老张)，通俗上来讲就是通过分数来告诉机器怎么说话。
...
上图中的N代表的是测试集中所包含的单词数(token)，V则代表测试集中不算重复的单词数（type），经过转换可以得道最后的公式，其中$\hat{p}(v_i)$的意思是$v_i$在测试集中出现的概率。最简单的思想就是我们的目标是最大化这里面出现的正确词的概率，preplexity表明了你预测正确词需要在其等效的数目中去选取，比如preplexity等于64的话，正确词选取的概率就是$\frac{1}{64}$，即$q(w_i)$增大的时候perplexity是减小的，可能这里还是不是很理解，但至少要理解上述所说的话。（可以去学习一下熵的概念，如果有人告诉我们一个相当不可能的时间发生了，我们收到的信息要多于我们被告知某个很可能发生的事件发生时收到的信息，如果我们知道某件事情一定会发生,那么我们就不会接收到信息。于是，我们对于信息内容的度量将依赖于概率分布 p(x) ，因此我们想要寻找一个函数 h(x) ，它是概率 p(x) 的单调递减函数，表达了信息的内容。）

还有种针对于机器翻译的评价标准，BLEU score，他的原理是：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c73b3ebb405ef75e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图system AB都是对reference的一个翻译，其中的precision就是不同长度的词组的匹配的占比，brevity penalty是怕如果一个翻译就只有reference中间的某一段，那么他的匹配占比就是100%（precision中的分母是针对翻译的句子本身），所以需要penalty来制约这种翻译长度较短的情况，那么我们可以给出以下公式：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-7629d53fadea2cf9.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

式中的右部指的就是precision，左部分就是指的brevity penalty，两者结合就是BLEU score。一般需要20以上才算可以。
...
**+1平滑**相当于在概率的基础上加上一个值，使得概率不会变为0，但语言模型一般不使用，效果不是太好。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-321bcf2645d92e6b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

**Back-off回退法**说的是当没出现出现的序列的情况下，我们可以简化序列，比如将Trigram的情况会出现有的概率为0的情况，则使用Brigram，如果还有概率为0的情况可以使用Unigram。

**Interpolate插值法**所用的方法是将Trigram、Brigram、Unigram一定比例混合使用，这样的话即使Trigram出现没有的序列而导致概率为0，其他几种情况的概率依旧可以使得最后的概率不为0,参数的选择是通过验证集来调整的，并且中间的$\lambda$是可以针对于全局的或者单个概率。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c77cd2c7a54edd82.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
**Modified Kneser-Ney Smoothing**则是对**Kneser-Ney Smoothing**又一改进，设置了不同的D值。![image.png](https://upload-images.jianshu.io/upload_images/7810235-454db205c3396495.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)意思是说现在的D值不仅仅是一个数，可能是多个阈值，在达到不同的阈值的情况下，在$c(w^i_{i-n+1})$的次数越多的情况下，阈值$D_i$会越来越大。
...
上面提到过在反向传播求梯度的过程中很容易出现梯度爆炸和消失的问题，一般的解决思路是：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-f9317170b6082c8d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1000)

下面我们会一个一个讲这些方式的具体意义。

**1.选择合适的激活函数**

![image.png](https://upload-images.jianshu.io/upload_images/7810235-310a24b462451eca.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

一般选择ReLu激活函数，可以发现该函数在求导后可以恒为1，这是我们想要的，一般不使用sigmod和tanh的原因是，两者在很大一个区间内求导所得的值都比较小，能使用的范围小。
...
就是之前讲到的规范，把每一个神经元都放在-1到1之间，避免爆炸或者消失。

**4.使用残差网络**
![image.png](https://upload-images.jianshu.io/upload_images/7810235-6b82aa3cf70854f8.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

使用残差网路很好的扩展了网络的结构，因为深层的网络中许多层其实应该是恒等映射的，即输入跟输出不变，但是机器很难正确的学习到那些应该恒等的层。我们的目标是让$H(x)=F(x)+x=x$，即输入和输出相同，所以我们只需要让式子中的$F(x)=0$就可以了，这里的$F(x)$即为残差。而学习$F(x)=0$更为简单一些，因为每层的参数初始值为0，更容易收敛。
...
![image.png](https://upload-images.jianshu.io/upload_images/7810235-696e19fa8a02b3b0.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

<表示预测文件，>表示目标文件，上面有行数的显示

#### 5.4.2 代码

代码目前是案例，在Linux上面可以跑（Python2.7在Windows环境下不怎么兼容TensorFlow）

GitHub地址：https://github.com/shixing/xing_nlp
...
![image.png](https://upload-images.jianshu.io/upload_images/7810235-d1ec80277b372e2d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

seq2seq作为现在还算主流的模型，它可以解决许多自然语言处理的基本任务，只需要将他的输入输出的形式做一些改变就可以。
...
上图为seq2seq的主要参数，**超参数**有层数（LSTM的层数）、hidden size（向量的维数）、$V_F$和$V_E$（F和E数据集单词个数）；**encoder**有input(输入，为矩阵的形式)、LSTM中间的参数（之前的知识可以知道中间的参数主要有f（遗忘门）、i（输入门）、o（输出门）、c（记忆细胞），这里面每个参数实际上有两个权值矩阵和一个偏执向量，所以一层LSTM包含$8d^2+4d$）；**decoder**有input（输入，为矩阵的形式）、LSTM中间的参数和output（里面有词向量组成矩阵和偏置向量），**计算量主要集中在这里**。 
...
我们发现想要计算出最大概率的序列，其实可以类比成上图这样的求路径的方式，最最简单的方式是通过贪心的策略来求解，但局部最优不一定能达到全局最优，所以这里采用了动态规划的思想，直接看上图就行了。他的时间复杂度是$O(V^2N)$，空间复杂度是$O(VN)$。可以看到动态规划的计算代价还是很大的，这里就提出了Beam Search来解决这个问题，需要注意的是LSTM没有马尔科夫假设。下面以一个列子作为讲解：
...
![image.png](https://upload-images.jianshu.io/upload_images/7810235-dadd26b9bedd7758.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

![image.png](https://upload-images.jianshu.io/upload_images/7810235-28940958299cc59a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上面的三张图分别是$a_t$、$c_t$和$\widetilde{h_t}$计算方式，$a_t$主要是算出$\bar{h_s}$和$h_t$的一个相似评分，当两者越相似的话，评分越高，最后对它进行softmax的操作，相当于算出了一个相似度的权重；而$c_t$主要是将求得的权重$a_t$和encoder得到的$\bar{h_s}$作求和，这样就突出了某一步的$\bar{h_s}$值；最后再将$c_t$和$h_t$做线性操作得到最终的预测结果。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-67292357ab883a63.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

另外，在做完attention的操作以后，将得出预测结果放到下次输入的时候，会出现负反馈的结果，比如以机器翻译为例，在X处我们得到“我”这个结果的$\widetilde{h_t}$，对应encoder处的“I”，那么将这里的$\widetilde{h_t}$作为输入再次放入decoder里面的时候，输出结果往往就不会再考虑“I”处了，这里其实不怎么理解...

![image.png](https://upload-images.jianshu.io/upload_images/7810235-aaeb1b831c29fed1.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

以上就是seq2seq涉及的一些使得模型优化的技巧，上面没有讲reverse的含义，操作就是将encoder中的序列给反过来，这样前面的单词就可以进行更少的步骤得到结果。BLEU是机器翻译的自动评估工具，值越大效果越好。 

## 6 FSM

状态机，下面主要是实现的例子的讲解，可能理论方面不太细节。

### 6.1 Finite State Acceptor

![image.png](https://upload-images.jianshu.io/upload_images/7810235-08ccc46309694f24.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

有限状态接受器的结构与seq模型比较相似，上图给出了他的结构状态，下面以例子说明：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-ef5b73e208b68e35.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图就是一个实际的例子，自动机需要接受{ab,ac,ad}来到达结束状态，其中有{1,2,3,}三个状态，3即为结束状态。$\delta$中就表示了状态可以通过何种方式来进行转移。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-9869f262ee3646d0.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

我们再看一个例子，这里我们可以清楚的看到，从起始到终止状态每种情况的都是我们所接受的，这就是自动机的构建。另外不能接受$a^nb^n$的FSA，因为如果n不加限制，那图就是无限大，显然不可能。

### 6.2 Finite State Transducer

有限状态转换器类似于seq2seq模型

![image.png](https://upload-images.jianshu.io/upload_images/7810235-a76df7c529aa6eed.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

如上图所示，是对应于seq2seq模型，下面给出例子说明：
![image.png](https://upload-images.jianshu.io/upload_images/7810235-fd40675aa416753b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图给出的例子对应关系已经非常清楚了，目的是为了实现大小写字母的转换，与acceptor不同的是他起始和结束状态都只有一个，$\delta$对应有一个输入（ex：a）和一个输出（ex：A）。

### 6.3 Noisy Channel Model 

即噪声信道模型，下面以一个具体的例子（POS词性标注）来说明

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c812c692ddc1e2fa.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

这里先以一个实际应用作讲解，part=of-speech简称POS，是词性标注的意思，他可以用监督方法也可以用无监督方法。监督方法可以使用seq模型或者CRF等，而无监督方法需要引入一下的概念：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-8e6dc11c54a7d6fd.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)k

可以从上图看到，无监督的方法是先生成不同的词性序列，再将句子中的词嵌套进去，$POS_i$有很多种情况，我们目的是求出每种情况的概率并求和，使得和最大化，但如何得到这些词性的序列，就需要通过EM算法。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-f14c5efd40fec96c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

首先我们还是先看看生成$POS_i$的过程，在这里我们可以用一个FSA来生成各种可能的词性序列。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-14204a60b02a0032.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

之后我们可以用一个FST来完成将词性序列替换成单词的过程。需要注意的是实际情况中，里面的转换都是有一个概率的，这个概率是要通过数据集进行训练的（由于是无监督学习，所以训练的数据集是没有标签的，仅是单纯的句子）。之后可以使用vietrbi算法得到最佳的一个词性序列。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-4f79faa09119c19e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

这里稍微讲一下HMM（隐马尔科夫模型），其实可以由一个FSA模型和一个FST模型表示，其中FSA种的状态转移概率形如$P=(V|P)$，而FST中的转换概率形如$P=(I|P)$。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c28bf3172ab9f87b.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

还是上面那张图，这里我们可以看到HMM本质上是求一个图的联合分布概率（图是带有方向的）。具体的表示可以如下图所示：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-64809c1c76264b69.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

这是上图所描述的一般表示形式，其中k代表的是所有输出y可能出现的情况。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-e2f43be68a2477db.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

这个就是一个实际的例子了，里面有属于FSA的状态转移以及属于FST的替换，概率p(I like you,ppp)就如图中走的某一条路径的各个概率的乘积。

### 6.4 EM算法

我们前面在6.3节讲过无监督学习下如何在序列标注问下最大化概率$p(sentence)$，这里我们就需要EM算法了。EM算法是用于求含有隐变量的概率模型参数的极大似然估计，可以直接看到结果的变量叫做观测变量，而隐变量是无法观测的。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-67c8bff4ef175206.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图中x即观测变量（对应目标检测中的实际的句子），而z则为隐变量（对应目标检测中的序列POS）。当我们想让$\sum p(x,z|\theta)$的概率最大化时候，很容易想到使用极大似然估计的方式，但对形如对数中间带求和的式子没有解析解，所以我们需要对式子进行一些处理，$\sum_z q(z)=1$。**上式中有一些错误，比如L的变形中的第二排的$p(x|\theta)$应改为$p(x,z|\theta)$。**

![image.png](https://upload-images.jianshu.io/upload_images/7810235-35775a7b5f26508d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

我们把左边称作下界，右边为散度的表现形式（这里不怎么理解），具体的思路是EM算法分为E（求期望）和M（求极大），我们首先让右式等于0，再求解左式的解析解（求导等于0），右式等于0的条件是$q(z)=p(z|x,\theta)$（这样的话$log1=0$），初始通过初始的$\theta_{old}$求得后验概率$q(z)$，再对左式进行求极大似然估计得到新的$\theta_{new}$，根据$\theta_{new}$求得新的$q(z)$，重复运算直至收敛。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-9a483551e02c38ac.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

这里以HMM具体应该EM算法的例子作为讲解，POS序列中包含x和y两个标签，字典V中只有a和b，我们要训练的数据是“aba”这句话。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-d5eb29cb45f8a76a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

图中代表的是所有序列对应的可能性，如果不好理解的话对照着6.3章最后一张图来看，$p(w_1|t1)$这种是序列到词的转换，而$p(t_2|t_1)$这种是状态（序列）之前的递进，整张表相当于初始化的$\theta$。这里的$p(t,w)$即将他们连乘，而将他们归一化以后即是我们所求的$p(z|x,\theta_{old})$。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-a10c2b7421463014.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

当我们算出$p(z|x,\theta_{old})$以后，对于这里的更新$\theta$的操作，过程如上，但没有理解原理。。。

## 7 CRF

### 7.1 一些NLP的应用

![image.png](https://upload-images.jianshu.io/upload_images/7810235-de050c5b8577bbcc.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图为NLP的一些应用，我们重点关注下命名实体识别（NER），其中PER代表人名；LOC代表地点；ORG代表组织；MISC代表其他。针对这四种类别可能出现多个词组合，提出了BIO标注方法，B代表当前实体开始的词，I代表实体中的词，O代表不在这个实体之中。

### 7.2 CRF理论

在下文中的描述，可能不够清楚，基础可以看博客：https://blog.csdn.net/juanjuan1314/article/details/78852205是针对于《统计学习方法》。

如果说之前的HMM是求一个联合概率的话，那么CRF（条件随机场）则是求一个条件概率。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-1c5fdd5b81af203a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

从式子中我们可以粗略的看到外部的$\frac{1}{z}exp$其实是归一化，里面的$\lambda$是要优化的参数，可以理解为权重。它计算的实际是在给定的一组输入的随机变量条件下，另一组输出随机变量的条件概率分布。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-55bddd66bf355874.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

CRF的核心就是里面的特征函数，针对于不同的状态，可以通过对$\lambda_i$的取值0或1来确定不同的特征函数，CRF理论上是可以构建无数个特征函数的。对比HMM，CRF可以支持更多的特征，而HMM只能依靠前一步的状态，但HMM可以支持无监督学习。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c0a40c8d89d37071.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

我们知道CRF的表现形式以后，我们需要学习参数$\lambda_i$。方法是使用梯度下降算法，先对$\lambda_i$求偏导，在加上学习率对参数进行更新。如何计算求偏导的后半部分是问题的难点（后半部分其实是特征函数$f$的期望）。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-b2c11060cdc3458a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图为计算的思路，跟HMM的思路比较像。 

## 8 句法分析（待补充）

这里主要以汉语为主，由于英语等语言都是以空格分开，所以汉语需要先进行分词，再进行其他操作，无形中增大了难度。对于句法分析这里主要介绍转换生成语法和依存句法。

之后还要讲语法分析，句子的语义结构是语言的意义的表达方式，句子的语法结构是语言信息传递的编码结构，句子同时受到语义和语法的共同影响，才形成了现实中千差万别的句子。

### 8.1 转换生成语法

由诺姆·乔姆斯基（Noam Chomsky,1928-)构建，“语言”在生成语法中界定为：

**句子的（有限或无限的）集，每个句子在长度上是有限的，它由结构成分有限的集构成。**

乔姆斯基对语言学最大的贡献就是**短语结构文法**，即：

![image.png](https://upload-images.jianshu.io/upload_images/7810235-253a37bc71b7ec85.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-b825e0245ecdab83.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上述定义比较清晰了，如果熟悉语法树的概念的话，可以知道X代表的是叶节点的实际的单词，而V代表的是非叶子节点的一些词性，例如NN代表名词短语、N代表名词等，R代表的则是关系。

#### 8.1.1 上下文无关文法

短语结构文法分为**上下文无关文法**和**上下文有关文法**，这里主要对上下文无关文法做研究，下面以一个具体的例子做说明

![image.png](https://upload-images.jianshu.io/upload_images/7810235-d77b5ae372bbce63.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

最上面为给定的文法规则，需要注意的是（2）写错了，应改为VP->VV,AS,NP，最后得到的结果称为推导树，也可以由图形式表示，即句法树。图形表示如下图

![image.png](https://upload-images.jianshu.io/upload_images/7810235-ddb7961ea6fc0cab.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

句法树是句子形式到句子语义识别的一个至关重要的环节，其精度还未达到所需的要求，其最终的标注形式还未统一。

#### 8.1.2 汉语句类

1 一般主谓结构

![image.png](https://upload-images.jianshu.io/upload_images/7810235-e2d45c44adc3ac96.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

2 “把”字结构

![image.png](https://upload-images.jianshu.io/upload_images/7810235-50b47f8c99db8102.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-9e3baaa6cabaffd4.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

3 “被”字结构

![image.png](https://upload-images.jianshu.io/upload_images/7810235-5c8136470d403198.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

4 致使结构

![image.png](https://upload-images.jianshu.io/upload_images/7810235-84498dbd47e8e14d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-0b4bbd92d12b5ac6.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

其他还有复合句的形式，在这里不做赘述，只是简单的做一下例子以便理解。

## 9 预训练模型

之前有介绍过word2vec，可以从无标注的文本中寻找到词的向量表示（词嵌入），这本身是一个比较好的方式来保留单词的语义信息（语义偏移）。但随着神经网络rnn到lstm再到注意力机制的发展，将词向量作为一些监督学习任务的“附属”结果似乎成为了主流，这种方式还是会存在一些问题，比如标注数据不足以及在编码解码的过程中无法考虑到双向的上下文信息。直到2018年ELMO、BERT、GPT的流行又让从无标注文本寻找词向量这一方式逐渐进入到人们的视野当中，其成功很大程度上要归功于通过预训练模型实现了迁移学习的概念。在自然语言处理的背景下，迁移学习本质上是在一个数据集上训练模型，然后对该模型进行微调，以在不同的数据集上执行不同的自然语言处理功能。

### 9.1 BERT

#### 9.1.1 Transformer

了解BERT的第一步是了解transformer，他没有使用传统的RNN已经其变种，完全使用自注意力机制来完成模型的构建。下面三张图表示了他的基本结构。
![image.png](https://upload-images.jianshu.io/upload_images/7810235-503e4ef87d7ebceb.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-51050424fb96fb2a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)![image.png](https://upload-images.jianshu.io/upload_images/7810235-43748b6e73fafd42.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

如上图1可以看到，transformer的本质是多层的seq2seq模型，只不过内部的结构并没有使用RNN，而单单采用自注意力模型以及全连接网络。图2以及图3展示了更为细致的内部结构，在encoder过程中，序列的每一个单词经过自注意力层的时候都要根据完整的序列才能得到最后的输出结果。下图表示了其中的具体操作步骤。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-aee33d4f895454c5.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

如上图所示，我们假定输入的第一个单词为thinking，将他词嵌入得到的向量分别乘与三个权重矩阵得到$q_1、k_1$和$v_1$，我们通过$q_1$和每一个单词的嵌入表示与权重矩阵得到的$k_i$进行内积，并除以8得到相似度权重（8为$k_i$的维度的平方根，$k_i$的维度默认为64，原文提出这样可以使得梯度更稳定），在通过softmax转换为概率（这里就是自注意力了），然后按权重与每一个$v_i$进行加权和最后得到$z_1$。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-b6e24a481f510814.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

在每个自注意力层可能不止存在一组权重矩阵，可以同时拥有多个权重矩阵来对不同的语义关系进行“关注”。那么每关注一种关系所得的结果都不尽相同，我们将这种多重的注意力机制叫做多头注意力机制。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-a5608cf3b67e22f8.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

上图为transformer的完整过程，通过多头注意力机制得到的多个向量进行拼接再乘与一个权重矩阵进行输出。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-fd314e32f8b94ccb.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

但是关于上面的流程忽略了一个重要的因素，那就是词序的问题，比如列子中的两句话，只通过之前的步骤是不会有区别的，所以要引入位置编码与之前的词嵌入进行整合。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-795b088ad533bbd7.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

对于如何给定位置编码，可以自己制定规则只要保证其信息独一无二就行，BERT采用的绝对位置编码（依靠学习出来的），即位置编码是靠学习出来的，而transformer采用的相对位置编码，其中pos为位置，i表示embedding的维度。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-3a21ae8a9bb34415.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

![image.png](https://upload-images.jianshu.io/upload_images/7810235-c8a9fc21b10096de.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

最后在每一个block中间加入一个残差连接和正则化。在整体的结构中在encoder和decoder之前同样使用了之前在seq2seq中使用过的注意力机制。可以发现transformer不仅是能够并行计算的，可以做的很深，并且很好的解决了之前RNN所无法解决的单向传入问题，有效的提取了上下文的信息，但仍无法解决数据集需要标注问题（监督学习）。

#### 9.1.2 BERT基本原理

![image.png](https://upload-images.jianshu.io/upload_images/7810235-cecae5a2a8bf3a7f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

首先我们了解下BERT的输入，输入与transformer比较相似都有：单词的embedding（token embeddings，可以叫WordPiece嵌入，能在单词的有效性和字符的灵活性之间取得一个折中的平衡，比如‘playing’被拆成play和ing）、位置的embedding以及之前没有的sentence embedding（区分输入属于第几个句子，BERT的输入为两个句子）。由于之前已有的ELMO和GPT其实都是将模型基于无监督的任务（语言模型）当中得到预训练的结果，这种方式虽然不需要数据标注，但语言模型本身是根据马尔科夫性来完成的，即只能向前搜索。而BERT把任务给修改成随机在句子中用mask代替单词，任务为预测被mask替代的单词是什么（masked language model，任务本身在1953年就提出），这样模型本身就会去真正考虑句子中的上下文信息。

![image.png](https://upload-images.jianshu.io/upload_images/7810235-cc354a2edd704014.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

关于为什么输入是两个句子其实是想让模型更多的训练出属于句子之间的关系，BERT在完成masked language model的任务同时，完成了第二个子任务，即判断输入的两个句子是否为连续出现的（Next sentence prediction）。两个子任务同时训练。

目前谷歌开放的模型有多个，前四个都是关于英文的，BERT-large-ml-cased是关于多国语言的，特别注意的是最后一个还有专门中文模型。uncased是考虑到英文的大小写情况，cased是全部转为小写。