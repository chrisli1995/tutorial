...
如上图所示，我们假定输入的第一个单词为thinking，将他词嵌入得到的向量分别乘与三个权重矩阵得到$q_1、k_1$和$v_1$，我们通过$q_1$和每一个单词的嵌入表示与权重矩阵得到的$k_i$进行内积，在通过softmax转换为概率（这里就是自注意力了），然后按权重与每一个$v_i$进行想加最后得到$z_1$。
